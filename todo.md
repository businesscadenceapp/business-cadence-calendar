...
## Interactive Hub Redesign — Liquid Glass + Sun/Moon Toggle
- [x] Create GlassHubNode component with liquid glass effect (backdrop-filter blur + gradient)
- [x] Implement sun/moon global toggle in center of hub (replaces ⚡/📈 center icons)
- [x] Add smooth state transitions (sun ↔ moon, circles dim/glow)
- [x] Add magnetic pull interaction (circles drift toward finger on pointer move)
- [x] Add ripple burst animation on tap
- [x] Add liquid fill animation on tap before navigating
- [x] Replace Co-Owner Inbox with Off the Clock (sun/moon toggle controls all hubs)
- [x] Persist Off-the-Clock state in localStorage
- [ ] Write vitest tests for hub state management
