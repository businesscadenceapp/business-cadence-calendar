...
describe("native mobile app icons", () => {
  it("uses a 1024 x 1024 iOS icon composed exactly from the approved heart", () => {
    expect(
    ...
    expect(JSON.parse(report)).toMatchObject({ composition_fidelity: "exact", size: [1024, 1024] });
  });
